...
//    抽象语法树的设计上，目前只有变量定义，常量，过程名写入到符号表，所以只需处理这几种情况
//**/
//void traverseAstTree(Symtable *table, ASTNode *node, int deep, char *fname)
//{
//    int i, res, curdeep, vnum = 0, dx = 3;
//    static int funSize[15] = {4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4};  //存储过程定义的活动记录空间 = dx + 变量个数, 初始值是4
//    ASTNode *childnode;
//
//    //最多嵌套15层
//    if (deep > 15)
...
//        if (node->type == CONSTANT)
//        {
//            res = addElement(table, "constant", INTEGER, fname, node->name, deep, node->data.val, 0);
//            printf("add constant:%s pos:%d \n", node->name, res);
//        }
//        else if (node->type == VAR)
//        {
//            vnum++;
//            funSize[deep]++;  //过程包含变量数++
//            //Symtable *table, char *type, char *ele, int lev, int val
//            res = addElement(table, "variable", INTEGER, fname, node->name, deep, dx+vnum, 0);
//            printf("add var:%s pos:%d \n", node->name, res);
//        }
//        else if (node->type == FUNCTION || node->type == SUBPROGRAM)
//        {
//            //deep 表示函数嵌套层次
...
//            {
//                //递归遍历
//                traverseAstTree(table, node->child[i], deep, fname);
//            }
//
//            //将变量生成完毕再将函数信息写到符号表
//            if (node->type == FUNCTION)
//            {
//                res = addElement(table, "function", STR, fname, node->name, curdeep, 0, funSize[deep]);
//                printf("add function:%s, pos:%d \n", node->name, res);
//            }
//
//            //将main函数默认加入符号表, main 函数的层级是0
//            if (deep == 1 && (node->type == SUBPROGRAM))
//            {
//                strcpy(node->name, "main");
//                res = addElement(table, "function", STR, fname, node->name, 0, 0, funSize[deep]);
//                printf("add function:%s, pos:%d \n", node->name, res);
//            }
//        }
//        else
//        {
//            //递归遍历
//            traverseAstTree(table, node->child[0], deep, fname);
//        }
//        node = node->sibling;
//    }
//}
